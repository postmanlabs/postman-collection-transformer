...
_.extend(Builders.prototype, {
    ...
    rawModeData: function (item) {
        ...
        if (mode === 'raw') {
            ...
            return _.get(item.request.body, 'file.src', E);
        }

        return E;  // v1 requires an empty string
    },
    ...
    request: function (item, collectionId, skipResponses) {
        ...
        request.queryParamData = item.request.queryParamData;
        request.headerData = item.request.headers || item.request.header;
        request.pathVariableData = item.request.url.variables || item.request.url.variable;
    ...
    description: function (descriptionV2) {
        if (_.isObject(descriptionV2) && !_.isEmpty(descriptionV2.content)) {
            return _.isString(descriptionV2.content) ? descriptionV2.content : E;
        }
        return _.isString(descriptionV2) ? descriptionV2 : E;
    }
});
