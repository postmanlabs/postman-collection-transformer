...
_.extend(Builders.prototype, {
    ...
    request: function (item, collectionId, skipResponses) {
        ...
        request.pathVariableData = urlObj.variables || urlObj.variable;

        request.queryParams = _.map(urlObj.query, function (queryParam) {
            queryParam = _.clone(queryParam);
            queryParam.enabled = !queryParam.disabled;
