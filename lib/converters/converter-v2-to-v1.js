...
_.extend(Builders.prototype, {
    ...
    requests: function (collectionV2) {
        var self = this,
            ...
            requests = [];
        _.each(_.filter(flattenedItems, function (item) {
            return !item.item; // only the items, not subfolders
        }), function (item) {
    ...
    folders: function (collectionV2) {
        ...
        var g = self.extractFolderItems(collectionV2);

        return _.map(g, function (folder) {
