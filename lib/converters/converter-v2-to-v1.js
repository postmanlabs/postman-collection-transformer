var _ = require('lodash'),
    ...
    populateIds = function (currentItem) {
        ...
        currentItem.items && currentItem.items.length && (currentItem.items = _.map(currentItem.items, populateIds));
        return currentItem;
    },
...
_.extend(Builders.prototype, {
    ...
    pathVariables: function (item) {
        var variable = _.get(item, 'request.url.variables');
    ...
    tests: function (item) {
        ...
        var events = _.filter(item.events, {listen: 'test'});
    ...
    preRequestScript: function (item) {
        ...
        var events = _.filter(item.events, {listen: 'prerequest'});
        return _.map(events, function (event) {
            var tests = _.get(event, 'script.exec');
            if (_.isArray(tests)) {
                tests = tests.join('\n');
    ...
    order: function (collectionV2) {
        var allItems = _.isArray(collectionV2.items) ? collectionV2.items : [collectionV2.items];
    ...
    folders: function (collectionV2) {
        return _.map(_.filter(collectionV2.items, function (element) {
            ...
            return {
                ...
                order: _.map(folder.items, function (f) {
