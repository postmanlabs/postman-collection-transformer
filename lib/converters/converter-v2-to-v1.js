...
_.extend(Builders.prototype, {
    ...
    headers: function (item) {
        ...
        // eslint-disable-next-line lodash/prefer-chain
    ...
    responses: function (item) {
        ...
        responses = _.map(allResponses, function (response) {
    ...
    requests: function (collectionV2) {
        var self = this,
            ...
            flattenedItems = _.flatten(_.map(collectionItems, function (item) { // TODO: upgrade to Lodash 4
                var folderItems = item.items || item.item;
                return (folderItems) ? folderItems : item;
            })),
            requests = [];
        _.forEach(flattenedItems, function (item) {
