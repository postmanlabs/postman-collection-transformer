...
_.extend(Builders.prototype, {
    ...
    itemGroups: function (collectionV1) {
        ...
        _.each(collectionV1.folders, function (folderV1) {
            ...
            // Get a reference to an itemGroup (using existing reference if it's already there
            itemGroup = itemGroupCache[folderV1.id] = (itemGroupCache[folderV1.id] || {
                /* jshint ignore:start */ // jscs:disable
                _postman_id: self.options.retainIds ? folderV1.id : undefined,
                /* jshint ignore:end */ // jscs:enable
                name: folderV1.name,
                description: folderV1.description,
                item: []
            });
            ...
            hasRequests && _.each(folderV1.order, function (requestId) {
                if (!self.request.cache[requestId]) {
                    // todo: what do we do here??
                    return;
                }

                itemGroup.item.push(self.singleItem(self.request.cache[requestId]));
            });
        });

        // Ideally, we could rely on collectionV1.folders_order, but since we need
        // self healing, better to skip it completely, and use our own detection logic, always.
        _.each(itemGroupCache, function (itemGroup) {
            !_.get(itemGroup, '_postman_isSubFolder') && items.push(itemGroup);
        });
