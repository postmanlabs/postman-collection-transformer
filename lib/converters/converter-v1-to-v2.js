var _ = require('lodash').noConflict(),
    ...
    _parseHeaders = function (data) {
        ...
        while (match) {
            headers[match[1]] = match[3].replace(regexes.trim, '$1');
            match = regexes.header.exec(data);
        }
        return headers;
    },
...
_.extend(Builders.prototype, {
    ...
    header: function (requestV1) {
        ...
        _.each(headerData, function (header) {
            if (_.startsWith(header.key, headersCommentPrefix) || header.disabled) {
        ...
        _.each(_parseHeaders(requestV1.headers), function (value, key) {
            if (!traversed[key]) {
                ...
                if (_.startsWith(key, headersCommentPrefix)) {
                    thisHeaderObject.disabled = true;
                    thisHeaderObject.key = thisHeaderObject.key.replace(headersCommentPrefix, '').trim();
                }

                headers.push(thisHeaderObject);
            }
        });
