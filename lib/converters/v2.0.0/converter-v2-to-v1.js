...
_.assign(Builders.prototype, {
    ...
    data: function (item) {
        ...
        if (mode === 'graphql' || mode === 'raw' || mode === 'file' || !mode) {
    ...
    /**
     * In case of GraphQl data this construct the proper graphQl data from a v2 item
     *
     * @param {Object} item - The v2 request to derive information form.
     * @returns {Object} - The inferred v1 request body mode
     */
    ...
    request: function (item, collectionId, skipResponses) {
        ...
        var units = ['headers', 'dataMode', 'data', 'rawModeData', 'graphqlModeData', 'pathVariables', 'tests',
                'preRequestScript', 'url'],
            self = this,
            request,
            description,
            currentHelper,
