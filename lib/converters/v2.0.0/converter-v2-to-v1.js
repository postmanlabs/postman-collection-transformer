...
_.assign(Builders.prototype, {
    ...
    /**
     * Determines the current auth helper for the provided v2 entity.
     *
     * @param {Object} entity - A v2.x collection, folder, or request.
     * @returns {String} - A unique v1 compliant identifier for the current auth helper.
     */
    currentHelper: function (entity) {
        return v2Common.authMap[entity && entity.auth && entity.auth.type];
    },

    /**
     * Determines the set of auth helper attributes for the provided v2 entity.
     *
     * @param {Object} entity - A v2.x collection, folder, or request.
     * @param {String} currentHelper - A unique v1 compliant identifier for the current auth helper.
     * @returns {Object} - A set of  v1 compliant auth parameters for the current auth helper.
     */
    helperAttributes: function (entity, currentHelper) {
        !currentHelper && (currentHelper = this.currentHelper(entity));

        var auth = entity && entity.auth,
            reference = util.authMappersFromCurrent[currentHelper];

        return auth && reference ? reference(auth[auth.type]) : {};
    },
    ...
    request: function (item, collectionId, skipResponses) {
        var units = ['headers', 'dataMode', 'data', 'rawModeData', 'pathVariables', 'tests', 'preRequestScript', 'url'],
            ...
            currentHelper = self.currentHelper(req),
            urlObj = _.isString(url) ? util.urlparse(url) : url,
            helperAttributes = self.helperAttributes(req, currentHelper);
        ...
        currentHelper && (request.currentHelper = currentHelper);
        _.size(helperAttributes) && (request.helperAttributes = helperAttributes);
    ...
    folders: function (collectionV2) {
        ...
        return _.map(self.extractFolderItems(collectionV2), function (folder) {
            var folderItems = folder.items || folder.item,
                ...
                tests = self.tests(folder),
                preRequestScript = self.preRequestScript(folder),
                currentHelper = self.currentHelper(folder),
                helperAttributes = self.helperAttributes(folder, currentHelper),
                result = {
            ...
            preRequestScript && (result.preRequestScript = preRequestScript);
            tests && (result.tests = tests);
            currentHelper && (result.currentHelper = currentHelper);
            _.size(helperAttributes) && (result.helperAttributes = helperAttributes);
...
module.exports = {
    ...
    convert: function (collection, options, callback) {
        ...
        var units = ['order', 'folders_order', 'folders', 'requests', 'tests', 'preRequestScript', 'currentHelper'],
