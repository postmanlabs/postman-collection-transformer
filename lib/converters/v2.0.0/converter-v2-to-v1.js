...
_.assign(Builders.prototype, {
     ...
     *
     * @param entityV2 {Object} - The v1 auth manifest to be transformed into v1.
     */
    auth: function (entityV2) {
        return util.authMapToArray(entityV2);
    },
    ...
    /**
     * Sanitizes variable definitions across v2 to v1 transformations.
     *
     * @param entityV2 {Object} - The v2 compliant entity from which to extract variable definitions.
     * @param env {Object} The optional set of environment overrides.
     * @return {Array}
     */
    ...
    folders: function (collectionV2) {
        ...
        return _.map(self.extractFolderItems(collectionV2), function (folder) {
            ...
            auth && auth[auth.type] && (result.auth = auth);
...
module.exports = {
    ...
    convert: function (collection, options, callback) {
        ...
        var units = ['order', 'folders_order', 'folders', 'requests'],
            ...
            auth = builders.auth(collection),
            description = builders.description(collection.info.description),
            variables = builders.variables(collection),
            events = builders.events(collection),
            newCollection = {
        ...
        auth && (newCollection.auth = auth);
        events && events.length && (newCollection.events = events);
        variables && variables.length && (newCollection.variables = variables);
