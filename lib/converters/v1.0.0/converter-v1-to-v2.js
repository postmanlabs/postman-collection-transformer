...
var _ = require('lodash').noConflict(),
    ...
    parseFormData = function (data, retainEmpty) {
        ...
        for (i = 0, ii = data.length; i < ii; i++) {
            // skip if:
            //  1. param is not an object, or
            //  2. param is missing property `key`
            // @note `key` is a required property, value can be null/undefined.
            if (typeof data[i] !== 'object' || _.isNil(data[i] && data[i].key)) {
                continue;
            }
