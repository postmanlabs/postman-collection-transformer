...
_.assign(Builders.prototype, {
    ...
    body: function (requestV1) {
        ...
        if (requestV1.dataOptions) {
            data.options = requestV1.dataOptions;
            // Convert v1 params mode to v2 formdata mode
            if (data.options && data.options.params) {
                data.options.formdata = data.options.params;
                delete data.options.params;
            }
            // Convert v1 binary mode to v2 file mode
            if (data.options && data.options.binary) {
                data.options.file = data.options.binary;
                delete data.options.binary;
            }
        }
