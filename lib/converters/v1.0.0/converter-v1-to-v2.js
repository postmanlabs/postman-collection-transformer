...
_.assign(Builders.prototype, {
    ...
    body: function (requestV1) {
        ...
        if (data.mode === 'raw') {
            ...
            data[data.mode] = { src: _.get(requestV1, 'rawModeData') };
        }
        else {
            data[data.mode] = _.map(requestV1.data, function (param) {
    ...
    singleResponse: function (responseV1) {
        ...
        response.id = self.options.retainIds && responseV1.id ? responseV1.id : util.uid();
