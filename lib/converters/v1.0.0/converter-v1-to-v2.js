...
_.assign(Builders.prototype, {
    ...
    body: function (requestV1) {
        ...
        // set null body if neither data|rawModeData|graphqlModeData nor dataMode is set,
        // or if the dataMode is explicitly set to null
        ...
        if ((!dataMode && _.isEmpty(requestV1.data) && _.isEmpty(requestV1.rawModeData) &&
            _.isEmpty(requestV1.graphqlModeData)) || requestV1.dataMode === null) {
        ...
        // set graphqlModeData if its a non empty object
        if (_.isObject(requestV1.graphqlModeData) && !_.isEmpty(requestV1.graphqlModeData)) {
        ...
        else if (graphqlModeData) {
            data.mode = 'graphql';
        }
        // set `formdata` if rawModeData is not set and data is an array
        // `data` takes higher precedence over `rawModeData`.
        else if (!rawModeData && !graphqlModeData && Array.isArray(requestV1.data || requestV1.rawModeData ||
            requestV1.graphqlModeData)) {
            data.mode = 'formdata';
        }
