...
var _ = require('lodash').noConflict(),
     ...
     * Returns an ordered array of `properties`, preserving the source order.
     * If parsing fails or input is invalid, returns null.
     *
     * @param {String} typeString - JSON string of type entries
     * @returns {?Object[]} - Ordered array of properties
     */
    parseTypeList = function (typeString) {
        ...
        try {
            ...
            return _.map(arr, function (entry) {
    ...
    /**
     * Annotates a list of items with `$type` using the same order as provided
     * in `typeList`. The `$type` value is a stringified JSON of properties.
     * Extra items or missing types are safely ignored.
     *
     * @param {Object[]} list - Array of items to annotate.
     * @param {?Object[]} typeList - Ordered array of properties corresponding to items
     */
    annotateWithTypeOrdered = function (list, typeList) {
        if (!typeList || !Array.isArray(list)) { return; }

        for (var i = 0; i < list.length; i++) {
            if (!typeList[i]) { continue; }

            try {
                list[i].$type = JSON.stringify(typeList[i]);
            }
            catch (e) {
                list[i].$type = typeList[i];
            }
        }
    };
...
_.assign(Builders.prototype, {
    ...
    url: function (requestV1) {
        ...
        // Annotate query params with `$type` in the same order
        if (requestV1.$type && requestV1.$type.queryParam) {
            annotateWithTypeOrdered(queryParams, parseTypeList(requestV1.$type.queryParam));
        }
        ...
        // Annotate path variables with `$type` in the same order
        if (requestV1.$type && requestV1.$type.pathParam) {
            annotateWithTypeOrdered(pathVariables, parseTypeList(requestV1.$type.pathParam));
        }
    ...
    header: function (requestV1) {
        ...
        // Annotate headers with `$type` in the same order
        if (requestV1.$type && requestV1.$type.headers) {
            annotateWithTypeOrdered(headers, parseTypeList(requestV1.$type.headers));
        }
    ...
    singleResponse: function (responseV1) {
        ...
        if (Object.prototype.hasOwnProperty.call(responseV1, '$type')) {
            ...
            // Annotate response headers in the same order when available
            if (Array.isArray(response.header) && responseV1.$type && responseV1.$type.headers) {
                annotateWithTypeOrdered(response.header, parseTypeList(responseV1.$type.headers));
            }
        }
