...
var _ = require('lodash').noConflict(),
    ...
    /**
     * Parses a JSON string representing an array of type entries of the form
     * [{ keyName: string, properties: Object }]
     * Returns a map of keyName -> properties.
     * If parsing fails or input is invalid, returns null.
     *
     * @param {String} typeString - JSON string of type entries
     * @returns {?Object} - Map of keyName to properties
     */
    parseTypeMap = function (typeString) {
        if (!typeString || !_.isString(typeString)) { return null; }

        try {
            var arr = JSON.parse(typeString);

            if (!Array.isArray(arr)) { return null; }

            return _.reduce(arr, function (acc, entry) {
                if (entry && entry.keyName && entry.properties) {
                    acc[entry.keyName] = entry.properties;
                }
    ...
    /**
     * Annotates a list of key/value items with `$type` when a matching entry
     * exists in typeMap. The `$type` value is a stringified JSON of properties.
     *
     * @param {Object[]} list - Array of items to annotate.
     * @param {?Object} typeMap - Map of keyName -> properties
     * @param {String} keyProp - Property name used as key in list items
     */
    annotateWithType = function (list, typeMap, keyProp) {
        if (!typeMap || !Array.isArray(list)) { return; }

        _.forEach(list, function (item) {
            var key = item && item[keyProp];

            if (key && Object.prototype.hasOwnProperty.call(typeMap, key)) {
                try {
                    item.$type = JSON.stringify(typeMap[key]);
                }
                catch (e) {
                    // Fallback to raw properties if stringify fails
                    item.$type = typeMap[key];
                }
            }
        });
    };
...
_.assign(Builders.prototype, {
    ...
    url: function (requestV1) {
        ...
        // Annotate query params with `$type` when available
        if (requestV1.$type && requestV1.$type.queryParam) {
            annotateWithType(queryParams, parseTypeMap(requestV1.$type.queryParam), 'key');
        }
        ...
        // Annotate path variables with `$type` when available
        if (requestV1.$type && requestV1.$type.pathParam) {
            annotateWithType(pathVariables, parseTypeMap(requestV1.$type.pathParam), 'key');
        }
    ...
    header: function (requestV1) {
        ...
        // Annotate headers with `$type` when available
        if (requestV1.$type && requestV1.$type.headers) {
            annotateWithType(headers, parseTypeMap(requestV1.$type.headers), 'key');
        }
    ...
    request: function (requestV1) {
        ...
        // Integrate body $type into body object when present, but do not keep top-level $type in output
        if (Object.prototype.hasOwnProperty.call(requestV1, '$type')) {
            if (requestV1.$type && Object.prototype.hasOwnProperty.call(requestV1.$type, 'body') &&
                request.body && (typeof request.body === 'object')) {
                request.body.$type = requestV1.$type.body;
            }
        }
    ...
    singleResponse: function (responseV1) {
        ...
        // Integrate response header/body type details, but do not keep top-level $type in output
        if (Object.prototype.hasOwnProperty.call(responseV1, '$type')) {
            // Also expose body type separately when present
            if (responseV1.$type && Object.prototype.hasOwnProperty.call(responseV1.$type, 'body')) {
                response.$type_body = responseV1.$type.body;
            }
            // Annotate response headers when available
            if (Array.isArray(response.header) && responseV1.$type && responseV1.$type.headers) {
                annotateWithType(response.header, parseTypeMap(responseV1.$type.headers), 'key');
            }
        }
