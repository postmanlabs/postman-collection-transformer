...
_.assign(Builders.prototype, {
    ...
    auth: function (requestV1) {
        var auth = Builders.super_.prototype.auth.call(this, requestV1);

        if (!(auth && auth.type && auth[auth.type])) { return; }

        return {
            type: auth.type,
            [auth.type]: _.map(auth[auth.type], function (value, key) {
...
module.exports = {
    ...
    convertSingle: function (request, options, callback) {
        ...
        try { converted = builders.singleItem(request); }
        catch (e) { err = e; }
    ...
    convertResponse: function (response, options, callback) {
        ...
        try { converted = builders.singleResponse(response); }
        catch (e) { err = e; }
