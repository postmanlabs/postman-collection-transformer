...
util = {
     ...
     * @param override {Object} - A set of optional overrides for variable transformations.
     * @return {Array} - The set of sanitized entity level variables.
     */
    handleVars: function (entity, override) {
        var source = entity && (entity.variables || entity.variable),
            result = _.map((override && override.values) || source, function (item) {
                var result = {
                    id: item.id || item.key,
                    value: item.value,
                    type: item.type || typeMap[typeof item.value] || 'any'
                };
