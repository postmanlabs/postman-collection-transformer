...
util = {
    ...
    handleVars: function (entity, options) {
        var self = this,
            noDefaults = options && options.noDefaults,
            source = entity && (entity.variables || entity.variable),
            fallback = options && options.fallback && options.fallback.values,
    ...
    authMapToArray: function (entity) {
        ...
        params = _.map(auth[auth.type], function (value, key) {
            return {
                key: key,
                value: value,
        ...
        params.length && (result[auth.type] = params);
        return result;
    },
    ...
    notLegacy: function (entityV1, type) {
        ...
        switch (type) {
            ...
            case 'auth':
                return !(_.has(entityV1, 'currentHelper') && entityV1.helperAttributes);
            default:
                return true;
        }
    },
