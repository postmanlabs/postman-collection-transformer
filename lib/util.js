...
util = {
    ...
    /**
     * Creates a merged instance from the provided arguments, giving more priority to the first argument.
     *
     * @param {Object[]} descData - The array of objects with description details included.
     * @param {Object[]|Object} parsedValues - The array of parsedValues (usually acquired from raw strings).
     * @return {Object[]} - The resultant merged objects.
     */
    merge: function (descData, parsedValues) {
        var result = _.clone(descData) || [],
            traversed = _.indexBy(descData, 'key');

        _.each(parsedValues, function (value, key) {
            if (!traversed[key]) {
                result.push({
                    value: value,
                    id: key
                ...
                traversed[key] = value; // prevent values with the same key down the line from overwriting current value
            }
        });
