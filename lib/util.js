...
util = {
     ...
     * @param callback
     */
    writeJSON: function (path, data, pretty, overwrite, callback) {
        var json;

        try {
            json = JSON.stringify(data, null, pretty ? 4 : 0);
        }
        catch (e) {
            return callback(e);
        }
        fs.writeFile(path, json, overwrite ? FSWF_FLAG_W : FSWF_FLAG_WX, callback);
    }
};
