...
util = {
    ...
    loadJSON: function (path, callback) {
        fs.readFile(path, function (error, data) {
            ...
            try {
                data = JSON.parse(stripJSONComments(data.toString()));
            }
            catch (err) {
                return callback(err);
            }
     ...
     * @param callback
     */
    writeJSON: function (path, data, options, callback) {
        var json = (options.pretty) ? JSON.stringify(data, null, 4) : JSON.stringify(data),
            flag = (options.overwrite) ? 'w' : 'wx';

        fs.writeFile(path, json, {flag: flag}, function (err) {
            if (err) {
                return callback(err);
            }
            callback();
