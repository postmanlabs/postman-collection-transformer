...
util = {
     ...
     * @param {?Boolean} options.nullifyEmptyValues - When set to true, empty property values will be set to null
        ...
        var self = this,
            ...
            result = _.map(source || fallback, function (item) {
                ...
                if (_.has(item, 'description')) {
                    result.description = (item.description && !nullifyEmpty) ? item.description : null;
                }
