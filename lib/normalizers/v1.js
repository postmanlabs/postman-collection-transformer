var _ = require('lodash').noConflict(),
    ...
    folderAuthOptions = { includeNoauth: true },
...
_.assign(Builders.prototype, {
    ...
    auth: function (entityV1, options) {
        ...
        if ((entityV1.auth === null) || (entityV1.currentHelper === 'normal') || (entityV1.currentHelper === null)) {
            return null;
        }
        ...
        if (currentHelper && !helperAttributes) { return; }
    ...
    request: function (requestV1, collectionId, skipResponses) {
        ...
        if (_.has(requestV1, 'currentHelper')) {
            ...
            if (!requestV1.currentHelper) {
                (requestV1.currentHelper !== null) && (delete requestV1.currentHelper);

                // @todo: Should currentHelper be recreated from helperAttributes.id if falsy?
                _.has(requestV1, 'helperAttributes') && !noDefaults && (delete requestV1.helperAttributes);
            }
        }

        auth = self.auth(requestV1);

        if (auth) { requestV1.auth = auth; }
        else if (auth === null) { // eslint-disable-line security/detect-possible-timing-attacks
            requestV1.auth = requestV1.currentHelper = null;
        }
    ...
    folders: function (collectionV1) {
        ...
        _.forEach(collectionV1.folders, function (folder) {
            ...
            auth = self.auth(folder, folderAuthOptions);
...
module.exports = {
    ...
    normalize: function (collection, options, callback) {
        ...
        try {
            ...
            (auth = builders.auth(collection)) ? (collection.auth = auth) : (delete collection.auth);
